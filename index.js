(async () => {
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message, heartbeat, pause }) => {
      ...
      console.log(
        chalk.green(
          `OUT: ${obj.WINDOW_START} - ${obj.WINDOW_END}: ${obj.USER_ID} | ${obj.PROP1} | ${obj.PROP2}`
        )
      );
    },
  });
  ...
  const send = async (i) => {
    ...
    console.log(
      chalk.yellow(
        `IN : ${value.timestamp}                         : ${value.properties.user_id} | ${value.properties.prop1} | ${value.properties.prop2}`
      )
    );
